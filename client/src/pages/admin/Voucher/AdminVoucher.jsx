...
const VouchersList = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "startDay",
      key: "startDay",
      render: (startDay) => new Date (startDay).toLocaleDateString ('vi-VN')
    },
    {
      title: "Ngày Kết Thúc",
      dataIndex: "endDay",
      key: "endDay",
      render: (endDay) => new Date (endDay).toLocaleDateString ('vi-VN')
    },
    {title: "Mã Voucher", dataIndex: "code", key: "code"},
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button icon={<Pencil size={24}/>} onClick={() => handleEditVoucher (record._id)}
                  className="text-yellow-500 border-none"/>
          <Button icon={<BadgeX size={24}/>} onClick={() => showDeleteModal (record._id)}
