...
const signUpOwner = async (req, res) => {
  const { name, password, email, phone, idenCard } = req.body;
  ...
  try {
    ...
    console.error("Error in signUpOwner:", error);
  ...
  if (customer) {
    if (customer.isActive === false) {
      return res.status (401).json ({
        login: false,
        message: `Tài khoản đã bị khóa vì: ` + customer.reasonInact + `   Liên hệ mở khóa tại: thymai.1510@gmail.com`
      });
    }
...
const registerCustomer = async (req, res) => {
  ...
  try {
    const customerExisted = await Customer.findOne ({email: email});
...
const strictSignInPartner = async (req, res) => {
  ...
  if (!existedPartner) {
    ...
    return res.status (200).cookie ('token', newToken, {httpOnly: true, secure: true, sameSite: "none"}).json ({
      login: true,
      role: 'owner',
      id: newPartner._id,
      name: newPartner.ownerName,
